...
import { AiFillHeart } from "react-icons/ai";
  ...
  export default function IndexPage() {
   ...
   //! Fetch events from the server ---------------------------------------------------------------
    useEffect(() => {
      ...
      axios
        .get("https://twooccasio.onrender.com/createEvent")
        .then((response) => {
  ...
  //! Like Functionality --------------------------------------------------------------
    const handleLike = (eventId) => {
      axios
        .post(`https://twooccasio.onrender.com/event/${eventId}`)
        .then((response) => {
    ...
    return (
      <>
      <div className="  bg-gradient-to-b from-primarydark to-primarylight mt-1 flex flex-col">
        <div className="hidden sm:block" >
          <div href="#" className="flex item-center inset-0">
            <img src="https://i.postimg.cc/K8xWKyJr/2.png" alt=""  className='w-full h-80'/> 
          </div>
        </div>
        ...
        {events.length > 0 && events.map((event) => {
          ...
          if (eventDate > currentDate || eventDate.toDateString() === currentDate.toDateString()){
            return (
              ...
              <div className='rounded-tl-[0.75rem] rounded-tr-[0.75rem] rounded-br-[0] rounded-bl-[0] w-full object-cover aspect-[16/9]'>
              <img
 src={event.image}
...
/>
                ...
                <button onClick={() => handleLike(event._id)}>
                  <AiFillHeart className="w-auto h-12 lg:h-10 sm:h-12 md:h-10 bg-white p-2 rounded-full text-primarydark shadow-md transition-all hover:text-red-600" />
                </button>
              ...
              <div className="m-2 grid gap-2">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2 items-center mr-4 text-red-600"> <AiFillHeart /> {event.likes}</div>
                </div>
